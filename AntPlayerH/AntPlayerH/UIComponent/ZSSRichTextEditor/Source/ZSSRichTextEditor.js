...
zss_editor.init = function() {
    ...
    $(document).on('click', '.video-timestamp-button', function(e) {
        e.preventDefault();
        e.stopPropagation(); // 阻止其他事件的影响
        var timestamp = $(this).data('timestamp');
        console.log("Timestamp clicked: " + timestamp);

        // 触发回调
        window.location = 'callback://timestamp/' + parseFloat(timestamp);
    });
...
zss_editor.addFlagButtonWithTimestamp = function(timestamp) {
    // 创建一个按钮元素，按钮内容为传入的时间戳
    var flagElement = `
        <button class="video-timestamp-button" data-timestamp="${timestamp}" style="
            ...
            cursor: pointer;
        ">
        <svg t="1723885427787" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1743" width="16" height="16"><path d="M153.6 554.688V1024H64V0h358.656l183.488 85.312H960V640H601.344L426.432 554.688H153.6z" fill="#262626" p-id="1744"></path></svg>
            <span style="display: inline-block; width: 1px; height: 16px; background-color: #000000; margin-right: 8px;"></span>
        <span style="color: #000000;">${timestamp}</span>
        </button>
    `;
    
    // 将按钮插入到富文本编辑器中
    zss_editor.insertHTML(flagElement);
};

//zss_editor.addTimestampClickListener = function() {
//    document.addEventListener('click', function(e) {
//        if (e.target && e.target.classList.contains('video-timestamp-button')) {
//            e.preventDefault();
